...
public class DomainExceptionFilter : ExceptionFilterAttribute
{
    ...
    protected virtual JsonResult Exception(Exception exception)
    {
        ...
        var error = new Error(exception.GetType().FullName, $"{exception.Message}: {exception.InnerException?.Message}");
    ...
    private string ObterMensagem(ItemInfoException exception)
    {
