     ...
     where TContext : IUnitOfWork<TContext>
{
    ...
    public virtual TEntity Update(TEntity entity, string[] ignoreProps)
    {
        var result = DbSet.Attach(entity);
        result.State = EntityState.Modified;
        var entry = ((DbContext)UnitOfWork).Entry(entity);
        ...
        {
            var property = entry.Metadata.FindProperty(prop);
            if (property != null)
    ...
    private object ObterParametroValor(QueryFilter.FilterContainer filterContainer, Type propertyType, bool nulable = false)
    {
        ...
        if (filterContainer.Value == null)
            ...
            result = System.Enum.Parse(propertyType.GetGenericArguments()[0], filterContainer.Value.ToString(), true);
    ...
    {
        SqlParameter[] parameters = ParameterCacheHelper.GetSpParameterSet(_connectionString, procedureName);
