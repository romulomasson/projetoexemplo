...
public static class IntExtensions
{
    ...
    {
        ThrowHelper.IfIsNull(action);
        ThrowHelper.IfLessThanOrEqZero(self);
    ...
    public static string Pluralization(this int self, string phrase)
    {
        ...
        foreach (Group group in result.Groups)
        {
            ...
            ThrowHelper.IfIsFalse(parts.Length == 3, nameof(parts.Length), new IndexOutOfRangeException(string.Format("Invalid content template: {0}", group.Value)));
